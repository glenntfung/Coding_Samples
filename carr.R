...
wind$Industry <- as.factor(wind$Industry)
...
calender <- subset(calender, select = c(Markettype, Clddt, Daywk, State))
colnames(calender)[colnames(calender) == "Clddt"] <- "TradingDate"
calender$TradingDate <- as.Date(calender$TradingDate)
calender$Markettype <- as.factor(calender$Markettype)
calender$Daywk <- as.factor(calender$Daywk)
...
# Assuming 'data' contains columns: 'stock_symbol', 'date', 'return', 'risk_free_rate', 'MKT', 'SMB', 'HML', 'UMD'
...
# Example data
# df1 <- your dataframe with 'date1' and 'symbol'
# df2 <- your dataframe with 'date2' and 'symbol'

library(dplyr)

# Ensure 'date1' and 'date2' are in Date format
...
for (i in indus) {
  carrtoprb[1, colnames(carrtoprb) == i] <- 
    ks.test(as.numeric(unlist(filter(carr.top.bm, Industry == i)[,8])), 
            as.numeric(unlist(filter(carr.top.rl, 
                                     Industry == i)[,8])))$statistic
  carrtoprb[2, colnames(carrtoprb) == i] <- 
    ks.test(as.numeric(unlist(filter(carr.top.bm, Industry == i)[,8])), 
            as.numeric(unlist(filter(carr.top.rl, 
                                     Industry == i)[,8])))$p.value
}

for (i in indus) {
  carrbotrb[1, colnames(carrtoprb) == i] <- 
    ks.test(as.numeric(unlist(filter(carr.bot.bm, Industry == i)[,8])), 
...
for (i in indus) {
  carrtb[1, colnames(carrtb) == i] <- 
    ks.test(as.numeric(unlist(filter(carr.bot.rl, Industry == i)[,8])), 
            as.numeric(unlist(filter(carr.top.rl, 
                                     Industry == i)[,8])))$statistic
